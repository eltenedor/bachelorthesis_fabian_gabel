...
\begin{displaymath}
  \sup_{|\alpha| < \infty, x \in \R^n} |D^\alpha u(x)| < \infty 
\end{displaymath}
benötigen.
...
\begin{bem}
  ...
  Gilt nun $u \in L^q(B_i)$ für alle $i = 1, \dots, n$, so gilt insbesondere $u \in \bigcup_{i = 1}^n L^q(B_i)$ und die Umkehrung der Aussage folgt aus der Inklusionsbeziehung $L^q(\bigcup_{i = 1}^n B_i) \subseteq L^q(K)$.
\end{bem}
