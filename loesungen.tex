...
\begin{proof}
  Wir halten zunächst fest, dass für alle Folgen $(v_n)_{n \in \N}$ mit $v_n \in C_0^\infty(\Omega)^m$ für alle $n \in \N$ und $\lim_{n \to \infty} \norm{v_n - v}_{1,q} = 0$ für ein $v \in \overline{C_0^\infty(\Omega)^n}^{\norm{\cdot}_{1,q}}$ auch $\lim_{n \to \infty} \norm{\div v_n - \div v} = 0$ gilt.
  ...
  Nach dem Satz von \banach\hyp{}\alaoglu\ ist die Einheitskugel in reflexiven Räumen schwach kompakt.
  Damit folgt, dass die beschränkte Folge $(\tilde u_j)_{j \in \N}$ eine schwach konvergente Teilfolge enthält mit Grenzwert $u \in L^q(\Omega)$.
  Zur Vereinfachung nehmen bezeichnen wir diese konvergente Teilfolge wieder mit $(\tilde u_j)_{j \in \N}$.
  ...
  Insbesondere gilt aufgrund der vorausgesetzten Beschränktheit von $\Omega$ auch $\mathbb{1} \in L^q(\Omega)$ und damit
  $$
  \langle u, \mathbb{1} \rangle = \int_{\Omega_0} u \d x = 0.
  $$
  ...
  In Abschnitt \ref{subsec:smoothMolly} haben wir gezeigt, das dies gerade impliziert, dass $u$ konstant ist.
